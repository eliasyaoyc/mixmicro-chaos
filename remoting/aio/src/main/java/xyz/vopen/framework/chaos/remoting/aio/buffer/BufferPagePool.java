...
public class BufferPagePool {
  ...
  protected ScheduledFuture future =
      BUFFER_POOL_CLEAN.scheduleWithFixedDelay(
          new Runnable() {
            @Override
            public void run() {
              if (enabled) {
   ...
   * @param isDirect 是否使用直接缓冲区
   */
  public BufferPagePool(
      final int pageSize, final int pageNum, final int sharedPageSize, final boolean isDirect) {
    if (sharedPageSize > 0) {
  ...
  static final class NoneBufferPagePool extends BufferPagePool {

    NoneBufferPagePool() {
      super(0, 0, false);
      bufferPages = new BufferPage[1];
      bufferPages[0] =
          new BufferPage(null, null, 0, false) {
            @Override
