 ...
 * 网络监控器，提供通讯层面监控功能的接口。
 *
 * <p>smart-socket并未单独提供配置监控服务的接口，用户在使用时仅需在MessageProcessor实现类中同时实现当前NetMonitor接口即可。
 ...
 * <b>注意:</b>
 *
 * <p>实现本接口时要关注acceptMonitor接口的返回值,如无特殊需求直接返回true，若返回false会拒绝本次连接。 <b>非必要情况下请勿使用该接口，未来可能会调整接口设计</b>
 *
 * @param <T> 消息对象实体类型
 * @author 三刀
 * @version V1.0.0
